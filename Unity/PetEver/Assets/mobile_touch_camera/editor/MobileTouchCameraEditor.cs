...
#region auto scroll damp
      ...
      DrawPropertyField("boundaryMode");
      DrawPropertyField("enableRotation");
      DrawPropertyField("enableTilt");
      const float minTiltErrorAngle = 0.001f;
      const float minTiltWarningAngle = 40;
      const float maxTiltErrorAngle = 90;
      if (mobileTouchCamera.EnableTilt == true) {
        DrawPropertyField("tiltAngleMin", mobileTouchCamera.TiltAngleMin >= minTiltErrorAngle, mobileTouchCamera.TiltAngleMin >= minTiltWarningAngle || mobileTouchCamera.BoundaryMode != BoundaryMode.DEFAULT, subSettingsInset);
        ...
        } else if (mobileTouchCamera.TiltAngleMin < minTiltWarningAngle && mobileTouchCamera.BoundaryMode == BoundaryMode.DEFAULT) {
          DrawErrorLine("Warning: The minimum tilt angle\nshould not be lower than " + minTiltWarningAngle + " in DEFAULT boundary mode.\nTry switching the boundary mode\nto CLAMP_POSITION to prevent computational errors.", Color.yellow);
        }
